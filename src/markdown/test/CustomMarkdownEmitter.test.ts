...
import { CustomMarkdownEmitter } from '../CustomMarkdownEmitter.js';
...
test('render Markdown from TSDoc', () => {
	...
	output.appendNodes([
		...
		new DocParagraph({ configuration }, [
			...
			new DocEmphasisSpan({ configuration, bold: true }, [new DocPlainText({ configuration, text: ' three ' })]),
	...
	output.appendNodes([
		...
		new DocParagraph({ configuration }, [
			new DocEmphasisSpan({ configuration, bold: true }, [new DocPlainText({ configuration, text: '*one*two*' })]),
			new DocEmphasisSpan({ configuration, bold: true }, [new DocPlainText({ configuration, text: 'three*four' })]),
		]),
	]);
