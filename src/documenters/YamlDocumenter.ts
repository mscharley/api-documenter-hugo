...
import { type DocBlock, type DocComment, type DocSection, StandardTags, StringBuilder } from '@microsoft/tsdoc';
import {
	ApiClass,
	type ApiConstructor,
	ApiDocumentedItem,
...
import type {
	...
	IYamlReferenceSpec,
	IYamlSyntax,
} from '../yaml/IYamlApiFile.js';
import type { IYamlTocFile, IYamlTocItem } from '../yaml/IYamlTocFile.js';
...
export class YamlDocumenter {
	...
	public constructor(apiModel: ApiModel, newDocfxNamespaces: boolean = false, yamlFormat: YamlFormat = 'sdp') {
	...
	private _populateYamlFunctionLike(
		...
		if (ApiReturnTypeMixin.isBaseClassOf(apiItem)) {
			...
			if (apiItem.tsdocComment?.returnsBlock) {
	...
	private _populateYamlVariable(uid: DeclarationReference, yamlItem: Partial<IYamlItem>, apiItem: ApiVariable): void {
		const syntax: IYamlSyntax = {
			content: apiItem.getExcerptWithModifiers(),
		};
		yamlItem.syntax = syntax;
	...
	private _populateYamlTypeAlias(uid: DeclarationReference, yamlItem: Partial<IYamlItem>, apiItem: ApiTypeAlias): void {
		const syntax: IYamlSyntax = {
			content: apiItem.getExcerptWithModifiers(),
		};
		yamlItem.syntax = syntax;
	...
	private _renderType(contextUid: DeclarationReference, typeExcerpt: Excerpt): string {
		...
		if (!excerptTokens.some((tok) => tok.kind === ExcerptTokenKind.Reference && Boolean(tok.canonicalReference))) {
			return typeName;
		}
		...
		const uid: string = contextUid
			.addNavigationStep('~' as Navigation, `${counter}`)
			.withMeaning('complex' as Meaning)
		...
		if (excerptTokens) {
			for (const token of excerptTokens) {
				if (token.kind === ExcerptTokenKind.Reference) {
					...
					const specUid: string | undefined = token.canonicalReference?.toString();
