...
import type { IYamlTocItem } from '../yaml/IYamlTocFile.js';
import type { IYamlItem } from '../yaml/IYamlApiFile.js';
import { YamlDocumenter } from './YamlDocumenter.js';

type ISnippetsFile = Record<string, string[]>;
...
export class OfficeYamlDocumenter extends YamlDocumenter {
	private readonly _snippets: ISnippetsFile;
	private readonly _snippetsAll: ISnippetsFile;

	// Default API Set URL when no product match is found.
	private readonly _apiSetUrlDefault: string = '/office/dev/add-ins/reference/javascript-api-for-office';

	// Hash set of API Set URLs based on product.
	private readonly _apiSetUrls: Record<string, string> = {
	...
	protected onCustomizeYamlItem(yamlItem: IYamlItem): void {
		...
		if (snippets) {
			...
			} else if (yamlItem.syntax?.return) {
