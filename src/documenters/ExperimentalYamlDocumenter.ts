...
import { ApiDocumentedItem, type ApiItem, ApiItemKind, type ApiModel } from '@microsoft/api-extractor-model';

import type { IConfigTableOfContents } from './IConfigFile.js';
import type { IYamlTocFile, IYamlTocItem } from '../yaml/IYamlTocFile.js';
import { YamlDocumenter } from './YamlDocumenter.js';
...
export class ExperimentalYamlDocumenter extends YamlDocumenter {
	private readonly _config: IConfigTableOfContents;
	private _tocPointerMap: Record<string, IYamlTocItem>;
	...
	private _filterItem(apiItem: ApiItem, tocItem: IYamlTocItem): void {
		...
		if (apiItem instanceof ApiDocumentedItem) {
			...
			const tagContent: string | undefined = docInlineTag?.tagContent.trim();
		...
		if (!filtered && this._catchAllPointer?.items) {
	...
	private _shouldNotIncludeInPointersMap(item: IYamlTocItem): boolean {
		...
		if (nonEmptyCategoryNodeNames?.length) {
