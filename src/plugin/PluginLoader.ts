...
import type { IApiDocumenterPluginManifest, IFeatureDefinition } from './IApiDocumenterPluginManifest.js';
import { MarkdownDocumenterFeature, type MarkdownDocumenterFeatureContext } from './MarkdownDocumenterFeature.js';
import { PluginFeatureInitialization } from './PluginFeature.js';
import type { DocumenterConfig } from '../documenters/DocumenterConfig.js';
	...
	public load(documenterConfig: DocumenterConfig, createContext: () => MarkdownDocumenterFeatureContext): void {
		...
		for (const configPlugin of documenterConfig.configFile.plugins || []) {
			try {
				...
				// Load the package
				const entryPoint: { apiDocumenterPluginManifest?: IApiDocumenterPluginManifest } | undefined = require(
				...
				if (!entryPoint.apiDocumenterPluginManifest) {
					throw new Error(
						'The package is not an API documenter plugin;' + ' the "apiDocumenterPluginManifest" export was not found',
					);
				}
				...
				if (manifest.manifestVersion !== 1000) {
					throw new Error(
						'The plugin is not compatible with this version of API Documenter;' + ' unsupported manifestVersion',
					);
				}
